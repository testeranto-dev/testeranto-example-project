...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for entry_point in entry_points {
        ...
        // Create a temporary binary target in src/bin/
        let src_bin_dir = workspace.join("src/bin");
        fs::create_dir_all(&src_bin_dir)?;
        
        let temp_bin_path = src_bin_dir.join(file_name);
        
        // Copy the entry point to src/bin/ as a binary target
        fs::copy(entry_point_path, &temp_bin_path)?;
        println!("  📝 Copied to src/bin/ as binary target");
        
        // Compile the binary
        println!("  🔨 Compiling with cargo...");
        let status = Command::new("cargo")
            .args(&["build", "--release", "--bin", base_name])
            .status()?;
        ...
        let source_bin = workspace.join("target/release").join(base_name);
...
"#, bundles_dir.display(), base_name);
        ...
        // Clean up: remove temporary binary from src/bin/
